function [x, y] = langevin()

    f2 = figure('Position', [1 1 900 800], 'Toolbar', 'figure', 'Visible', 'off', 'MenuBar', 'none', 'Name', 'Langevin Random Walk in Closed 2D');
    movegui(f2, 'center')
    ...
    function updateGraph(source, eventdata)
        ...
        eta  = 1.562e-5;              % viscosity of air in SI units (Pascal-seconds) at 293 K
        ...
        vX = sqrt(kB * T / m);
        vY = vX;
        dT = str2num(get(editT, 'String')) * tau;
        ...
        if get(rUntilExit, 'Value') == 1
            ...
            while ~(x(i) > -wTop/2 && x(i) < wTop/2 && y(i) >= L)
                ...
                vX = [vX, vX(i-1)*exp(-dT/tau) + (D * (1 - exp(-2 * dT / tau)) / tau)^.5 * randn(1)];
                vY = [vY, vY(i-1)*exp(-dT/tau) + (D * (1 - exp(-2 * dT / tau)) / tau)^.5 * randn(1)];

                x = [x, x(i - 1) + vX(i - 1) * tau * (1 - exp(-dT / tau)) + (D * (1 - exp(-dT/tau)) ^ 3 / (1 + exp(-dT/tau)))^.5 * randn(1) + (2 * D * (dT + tau * (1 - exp(-dT/tau)) * (-2 + .5 * (1 + exp(-dT/tau)) - (1 - exp(-dT/tau))^2 / (2 * (1 + exp(-dT/tau))))))^.5 * randn(1)];
                y = [y, y(i - 1) + vY(i - 1) * tau * (1 - exp(-dT / tau)) + (D * (1 - exp(-dT/tau)) ^ 3 / (1 + exp(-dT/tau)))^.5 * randn(1) + (2 * D * (dT + tau * (1 - exp(-dT/tau)) * (-2 + .5 * (1 + exp(-dT/tau)) - (1 - exp(-dT/tau))^2 / (2 * (1 + exp(-dT/tau))))))^.5 * randn(1)];
