...
%         if get(rUntilExit, 'Value') == 1
            ...
            while nextBool == false && ~(x(i) > -wTop/2 && x(i) < wTop/2 && y(i) >= L) && ~(x(i) > -wBot/2 && x(i) < wBot/2 && y(i) < 0) 
                ...
                if ~(x(i) > -wTop/2 && x(i) < wTop/2 && y(i) >= L) && ~(x(i) > -wBot/2 && x(i) < wBot/2 && y(i) <= 0) && nextBool == false
                    if x(i) < -W/2
...
% %                     else
                        walk = {{x, y}};
                        if length(allWalks) == 0
                            allWalks = {walk};
                        elseif length(allWalks) < arraySpot
                            allWalks = [allWalks, {walk}];
                        else
                            allWalks{arraySpot} = [allWalks{arraySpot}, walk];
                        end
...
%                     end
            ...
            set(editN, 'String', num2str(steps));
...
%             end
        ...
        for subi = 1:length(allWalks)
            
            subplot(length(allWalks),1, subi)
            plot(-wTop/2, L, 'ro', wTop/2, L, 'ro', -wBot/2, 0, 'ro', wBot/2, 0, 'ro');
            hold on;
            plot([-W/2,-W/2,W/2,W/2,-W/2], [0, L, L, 0, 0], 'r');
            hold on;
            for subj = 1:length(allWalks{subi})
                subj
                x = allWalks{1,subi}{1, subj}{1, 1}
                y = allWalks{1,subi}{1, subj}{1, 2};
                plot(x, y);
                hold on;
                axis([-W/2 W/2 0 L]);
            end
